                          ...
                          const char *value)
{
    ...
    lv_label_set_text(cta, "TAP TO SHIFT  >");
...
void flow_ui_home_create(lv_obj_t *root, const flow_app_state_t *state)
{
    ...
    add_card_text(energy_card, "ENERGY", energy_value);
    ...
    add_card_text(style_card, "STYLE", state->snapshot.current.style);
