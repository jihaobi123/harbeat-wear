...
找不到 Hub 时，连接页可以点中央的 `VIEW HOME` 进入只读主页。后台仍会继续搜索 Hub。首次启动还没有可信状态时，主页以 `03 / 05`、`HIPHOP`、`96 BPM` 渲染完整人物插画，并标成 `PREVIEW`；这些值只用于保持 UI 完整，不代表现场状态。如果之前同步过，则显示最后一次可信状态。透明触摸层会阻止离线控制和发送。
